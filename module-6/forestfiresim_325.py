...
# Forest Fire Simulation: Program

# The purpose of this program is to simulate how wildfires spread through a forest,
# now including a lake feature that acts as a natural firebreak.
...
TREE = 'A'
FIRE = '@'
WATER = '*' # Added new character for Lake
EMPTY = ' '
    ...
    while True:  # Main program loop.
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                # If the current cell is water, copy it to the nextForest and skip other logic
                if forest[(x,y)] == WATER:
                    ...
                    for ix in range(-1, 2):
                        for iy in range(-1, 2):
                            neighbor = (x + ix, y + iy)
                            # Fire spreads to trees only if they are not part of the lake
...
def createNewForest():
    ...
    # Adding lake in the center of the forest
    lake_start_x = WIDTH // 3
    lake_end_x = lake_start_x + 10
    lake_start_y = HEIGHT // 3
    lake_end_y = lake_start_y + 5

    for x in range(WIDTH):
        for y in range(HEIGHT):
            if lake_start_x <= x <= lake_end_x and lake_start_y <= y <= lake_end_y:
                forest[(x, y)] = WATER  # Lake
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('blue') # Display lake in blue
                print(WATER, end='')
            elif forest[(x, y)] == EMPTY:
                print(EMPTY, end='')
        print()
